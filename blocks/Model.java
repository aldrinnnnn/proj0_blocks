...
class Model {
    ...
    boolean placeable(Piece piece, int row, int col) {
        ...
        for (int r = row; r < row + piece.height(); r++) {
            for (int c = col; c < col + piece.width(); c++) {
                if (get(r, c) && piece.get(r - row, c - col)) {
                    return false;
                }
            }
        }
    ...
    boolean placeable(Piece piece) {
        for (int r = 0; r < _cells.length; r++) {
            for (int c = 0; c < _cells[0].length; c++) {
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        int cellsHeight = ncols * _height;
        int cellsWidth = nrows * _width;
        if (nrows == 0) {
            return cellsHeight + (_streakLength * cellsHeight);
        } else if (ncols == 0) {
            return cellsWidth + (_streakLength * cellsWidth);
        } else {
            return (cellsWidth + cellsHeight - (nrows * ncols))
                    + (_streakLength * (cellsWidth + cellsHeight));
        }
    }
